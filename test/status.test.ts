...
describe("buildStartupNotice", () => {
	test("names listen mode when it is on, and how to stop it", () => {
		const notice = buildStartupNotice(card(), { enabled: true, intervalSeconds: 60 });
		assert.match(notice.message, /listen mode ON/);
		assert.match(notice.message, /\/teams-listen off/);
	});

	test("says the state plainly when it is off", () => {
		const notice = buildStartupNotice(card(), { enabled: false, intervalSeconds: 60 });
	...
	test("the surprising state is the louder one", () => {
		...
		assert.equal(buildStartupNotice(card(), { enabled: true, intervalSeconds: 60 }).level, "warning");
		assert.equal(buildStartupNotice(card(), { enabled: false, intervalSeconds: 60 }).level, "info");
	});
	...
	test("copes with a signed-in account that has no user name yet", () => {
		const notice = buildStartupNotice(card({ user: undefined }), { enabled: false, intervalSeconds: 60 });
		assert.match(notice.message, /loaded \(work, safety: open\)/);
	});

	test("reports the interval it will actually poll at", () => {
		const notice = buildStartupNotice(card(), { enabled: true, intervalSeconds: 15 });
